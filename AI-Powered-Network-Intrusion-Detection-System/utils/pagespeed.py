    ...
    def _parse_results(self, data: Dict, url: str, strategy: str) -> Dict[str, Any]:
        ...
        try:
            ...
            core_vitals = {
                ...
                'inp': ['interaction-to-next-paint', 'first-input-delay'],
                'cls': ['cumulative-layout-shift']
            }

            for vital_name, audit_names in core_vitals.items():
                vital_data = {}
                for audit_name in audit_names:
                    candidate = audit_results.get(audit_name, {})
                    if candidate.get('numericValue') is not None or candidate.get('displayValue'):
                        vital_data = candidate
                        break
