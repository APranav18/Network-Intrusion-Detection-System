...
@dashboard_bp.route('/dashboard/clear-all', methods=['POST'])
@csrf.exempt
@login_required
def clear_all_dashboard_data():
    """Admin-only: Clear all alerts and network flows from the dashboard."""
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'success': True, 'alerts_deleted': num_alerts, 'flows_deleted': num_flows})
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'error': str(e)}), 500
