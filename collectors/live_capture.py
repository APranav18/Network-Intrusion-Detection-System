...
class IPRealtimeCallback(PacketCallback):
    """Callback that scores IPs in real time as packets arrive."""

    def __init__(self, alert_callback: Optional[Callable] = None, use_cache: bool = True, flask_app=None):
        self.alert_callback = alert_callback
        self.use_cache = use_cache
        self.flask_app = flask_app
        self.alerts: List[Dict[str, Any]] = []
        self._last_status_by_ip: Dict[str, str] = {}

    def on_packet(self, packet: CapturedPacket) -> None:
        for role, ip in (('source', packet.src_ip), ('destination', packet.dst_ip)):
            self._score_ip(packet, ip, role)

    def _score_ip(self, packet: CapturedPacket, ip: str, role: str) -> None:
        try:
            from utils.ip_detector import analyze_ip

            if self.flask_app is not None:
                with self.flask_app.app_context():
                    result = analyze_ip(ip, use_cache=self.use_cache)
            else:
                result = analyze_ip(ip, use_cache=self.use_cache)
            assessment = result.get('attack_assessment', {}) if isinstance(result, dict) else {}
            status = assessment.get('status')

            if status not in {'attacked', 'suspicious'}:
                return

            previous_status = self._last_status_by_ip.get(ip)
            if previous_status == status:
                return

            self._last_status_by_ip[ip] = status
            ...
            alert = {
                ...
                'role': role,
                'status': status,
                'status_label': assessment.get('status_label'),
                'probability': assessment.get('realtime_ai_assessment', {}).get('probability'),
                'confidence': assessment.get('realtime_ai_assessment', {}).get('confidence'),
                'detection_mode': assessment.get('detection_mode'),
            ...
            logger.error(f"Realtime IP scoring error: {e}")
...
class LiveCaptureManager:
    ...
    def __init__(self, detector=None, alert_callback: Optional[Callable] = None, enable_ip_monitor: bool = True, flask_app=None):
        ...
        self.ip_realtime_callback = IPRealtimeCallback(alert_callback=alert_callback, flask_app=flask_app)
        self.enable_ip_monitor = enable_ip_monitor
        ...
        if self.enable_ip_monitor:
            self.capture.add_callback(self.ip_realtime_callback)
    ...
    def get_alerts(self) -> List[Dict]:
        ...
        alerts = list(self.detection_callback.alerts)
        alerts.extend(self.ip_realtime_callback.alerts)
        return alerts
